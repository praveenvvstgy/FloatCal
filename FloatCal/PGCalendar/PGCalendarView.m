...
- (instancetype) init {
	self = [super initWithNibName:@"PGCalendarView" bundle:[NSBundle bundleForClass:[self class]]];
	if (self != nil) {
		[self commonInit];
	}
...
- (void)viewDidLoad {
	...
	for(int row = 0; row < 6;row++) {
		for(int col = 0; col < 7; col++) {
			...
			PGCalendarCell* cell = [self viewByID:_id];
			cell.target = self;
...
//	self.date = [NSDate date];
	PGCalendarBackground* bv = (PGCalendarBackground*)self.view;
...
- (void) setSelectedDate:(NSDate *)selectedDate {
	...
	for(int row = 0; row < 6;row++) {
		for(int col = 0; col < 7; col++) {
			PGCalendarCell*cell = self.dayCells[row][col];
			BOOL selected = [PGCalendarView isSameDate:cell.representedDate date:_selectedDate];
	...
	for(int row = 0; row < 6;row++) {
		for(; col < 7; col++) {
			if(day <= lastDay) {
				...
				BOOL selected = [PGCalendarView isSameDate:d date:_selectedDate];
				cell.selected = selected;
				day++;
			}
		}
